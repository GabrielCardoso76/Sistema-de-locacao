...
function renderDashboard() {
  ...
  todaysRentals.forEach(rental => {
    ...
    // Get rented items summary (optional, but nice)
    // const items = mockItensAlugados
    //   .filter(ia => ia.aluguel_id === rental.id)
    //   .map(ia => {
    //      const item = mockItems.find(i => i.id === ia.item_id);
    //      return item ? `${ia.quantidade}x ${item.nome}` : '';
    //   }).join(', ');
