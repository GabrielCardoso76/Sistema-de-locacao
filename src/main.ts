  ...
  try {
    ...
    checkinButtons.forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            if (rentalId) {
                const currentRentals = await dataService.getRentals();
    ...
    cancelButtons.forEach(btn => {
            ...
            if (rentalId) {
                ...
                if (confirm(`Tem certeza que deseja CANCELAR o aluguel de ${rental.cliente_nome}?`)) {
                    try {
                        ...
                        alert('Erro ao cancelar aluguel. Tente novamente.');
                        console.error(error);
                    }
                }
            }
      ...
      app.innerHTML = '<p class="text-center text-red-500 mt-10">Erro ao carregar dados.</p>';
  }
}
