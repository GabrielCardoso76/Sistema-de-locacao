...
export async function renderCheckIn(container: HTMLElement, rental: Aluguel, onBack: () => void) {
  container.innerHTML = '<p class="text-center mt-10">Carregando...</p>';

  try {
    const allItems = await dataService.getItems();
    const rentedItems = await dataService.getRentedItems(rental.id);
    ...
    rentedItems.forEach(ri => {
        const item = allItems.find(i => i.id === ri.item_id);
        if (!item) return;
        ...
        <div class="grid grid-cols-3 gap-2 text-center text-sm">
            <div>
                ...
                <input type="number" min="0" max="${ri.quantidade}" value="${ri.quantidade}" data-type="limpo" data-id="${item.id}" class="w-full p-2 border rounded text-center input-checkin">
            </div>
            <div>
                <label class="block text-yellow-600 font-bold mb-1">Sujo</label>
                <input type="number" min="0" max="${ri.quantidade}" value="0" data-type="sujo" data-id="${item.id}" class="w-full p-2 border rounded text-center input-checkin">
            </div>
            <div>
                <label class="block text-red-600 font-bold mb-1">Quebrado</label>
                <input type="number" min="0" max="${ri.quantidade}" value="0" data-type="quebrado" data-id="${item.id}" class="w-full p-2 border rounded text-center input-checkin">
            </div>
        </div>
        <p id="error-${item.id}" class="text-xs text-red-600 font-bold mt-2 hidden text-center">Soma incorreta! Total deve ser ${ri.quantidade}</p>
        `;
    ...
    function validate() {
        let isValid = true;
        rentedItems.forEach(ri => {
            // Note: Changed data-id to be item.id for simplicity in lookup
            const limpoInput = document.querySelector(`input[data-id="${ri.item_id}"][data-type="limpo"]`) as HTMLInputElement;
            const sujoInput = document.querySelector(`input[data-id="${ri.item_id}"][data-type="sujo"]`) as HTMLInputElement;
            const quebradoInput = document.querySelector(`input[data-id="${ri.item_id}"][data-type="quebrado"]`) as HTMLInputElement;
            const errorMsg = document.getElementById(`error-${ri.item_id}`);

            if (limpoInput && sujoInput && quebradoInput) {
    ...
    finishBtn.addEventListener('click', async () => {
        ...
        finishBtn.textContent = 'Salvando...';

        try {
            const returns = rentedItems.map(ri => {
                const limpo = parseInt((document.querySelector(`input[data-id="${ri.item_id}"][data-type="limpo"]`) as HTMLInputElement).value || '0');
                const sujo = parseInt((document.querySelector(`input[data-id="${ri.item_id}"][data-type="sujo"]`) as HTMLInputElement).value || '0');
                const quebrado = parseInt((document.querySelector(`input[data-id="${ri.item_id}"][data-type="quebrado"]`) as HTMLInputElement).value || '0');
                return { itemId: ri.item_id, limpo, sujo, quebrado };
            });

            await dataService.checkInRental(rental.id, returns);
            ...
            finishBtn.textContent = 'Finalizar Check-in';
        }
    });
  } catch (error) {
